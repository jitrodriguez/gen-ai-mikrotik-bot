...
class ContextView:
    ...
    def draw(self,products=[],delete_callback=None):
        ...
        with self.container:
            if len(products) > 0:
                ...
                for i, product in enumerate(products):
                    with cols[i]:
                        self.draw_product(product, f"{i}-context-product",delete_callback=lambda i=i: delete_callback(i))
            else:
    ...
    def draw_product(self,product,id,delete_callback=None):
        product = Product(
            ...
            secondary_button_callback=delete_callback
        )
        product.draw(id=id)